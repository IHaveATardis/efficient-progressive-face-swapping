...
def getarcface(pretrained=None):
    """
    获取 ArcFace 模型并返回全局 ID 特征。
    :param pretrained: 预训练权重路径。
    :return: ArcFace 模型，输出全局特征 [B, 512]。
    """
    model = iresnet100().eval()
    for param in model.parameters():
        param.requires_grad = False
    ...
    # 不替换 forward 方法，直接调用原始方法
    def forward_with_global_id(x, return_id512=True):
        return IResNet.forward(model, x, return_id512=return_id512)

    model.forward = forward_with_global_id
    return model
