...
class AttnProcessor2_0(nn.Module):
    ...
    def __call__(
        ...
        # if encoder_hidden_states is None:
        #     print("in attn processor, encoder_hidden_state is None")
        # else:
        #     print("in attn processor, encoder_hidden_state shape=", encoder_hidden_states.shape)
...
class IDAttnProcessor2_0(torch.nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, lora_r=64, lora_alpha=64):
        super().__init__()
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("AttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
