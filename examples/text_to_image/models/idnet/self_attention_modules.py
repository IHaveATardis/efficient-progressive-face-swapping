        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "write":
                    ...
                    # 检查 bank 中的梯度状态
                    for idx, tensor in enumerate(self.bank):
                        print(f"[Debug] Bank[{idx}] requires_grad={tensor.requires_grad}, shape={tuple(tensor.shape)}")
                    ...
                    # 检查拼接后的梯度状态
                    print(f"[Debug] ref_hidden_states requires_grad={ref_hidden_states.requires_grad}, shape={tuple(ref_hidden_states.shape)}")
                    ...
                    # 检查 attention 输出的梯度状态
                    print(f"[Debug] ref_states requires_grad={ref_states.requires_grad}, shape={tuple(ref_states.shape)}")
                    ...
                    # 检查最终输出是否还在计算图里
                    print(f"[Debug] attn_output_add.requires_grad={attn_output_add.requires_grad}")
