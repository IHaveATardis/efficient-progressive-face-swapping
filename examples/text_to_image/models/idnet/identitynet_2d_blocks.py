...
from .identitynet_transformer_2d import Transformer2DModel
    ...
    def forward(
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = self._gradient_checkpointing_func(create_custom_forward(resnet), hidden_states, temb)
            else:
                hidden_states, idnet_output = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
                    attention_mask=attention_mask,
                ...
                hidden_states = resnet(hidden_states, temb)
                idnet_residuals += idnet_output

        return hidden_states, idnet_residuals
...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        return hidden_states, output_states, idnet_residuals
