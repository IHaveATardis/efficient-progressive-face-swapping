...
from modules.id_encoder import ID2Token
from modules.attr_encoder import Image2Token

from modules.attention_processor import AttnProcessor2_0 as AttnProcessor
from modules.attention_processor import IDAttnProcessor2_0 as IDAttnProcessor

from ffhq_celeba_train.dataset.multi_domain_pair_dataset import DomainSpec, SingleCelebAPairDataset, SingleFFHQPairDataset
from ffhq_celeba_train.dataset.multi_domain_pair_dataset import collate_fn as _orig_collate_fn
...
def collate_fn(examples):
    ...
    """
    batch = _orig_collate_fn(examples)  # 原逻辑：只打包张量
    metas = []
    for ex in examples:
        # __getitem__ 返回的字段名
        metas.append({
    ...
    batch["__meta__"] = metas  # 非张量，训练计算中不会用到，不会被 .to(device)
    return batch
...
def main():
    ...
    # Enable two mode control for attention layers
    ...
    # Keep id_encoder and attr_encoder in fp32 to avoid fp16 gradient unscale issues.

    # Add adapter and make sure the trainable params are in float32.
    unet.add_adapter(unet_lora_config)

    # === 解冻 attn_processors 中自定义 cross-attn 的参数 ===
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(model, type(unwrap_model(unet))):
                    ...
                    unet_to_save = model.state_dict()  # 保存整个 unet 的权重
    ...
    # —— DDP友好：设置epoch=0，构建初始index_map —— #
    train_dataset.set_epoch(0)

    # === Debug: 打印数据规模 & 期望的 steps/epoch ===
    if accelerator.is_main_process:
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=False,  # 重要：不要再让DataLoader洗牌
    ...
    # —— 采样日志配置（仅主进程写），与 loss.csv 同目录 —— #
    sample_log_limit = 2000     # 仅记录前 2000 个 batch
    sample_log_count = 0
    sample_log_buffer = []
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        unet.train()
        train_loss = 0.0
        # 让每个epoch重建index_map并洗牌（所有rank一致）
        train_dataset.set_epoch(epoch)
        for step, batch in enumerate(train_dataloader):
            # —— 记录本 batch 的样本构成（仅主进程；低 IO：缓冲后分批落盘）——
            ...
            with accelerator.accumulate(unet):
                ...
                # Get the id embedding for conditioning
            ...
            if accelerator.sync_gradients:
                ...
                # 把 loss 记到 CSV（根目录：output_dir/train_logs/loss.csv）
                if accelerator.is_main_process:
                    # 记录 loss 到 CSV
